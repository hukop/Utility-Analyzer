...
pub fn render_cost_heatmap(ui: &mut Ui, data: &ElectricData, state: &mut HeatmapState) {
    ...
    ui.heading("Daily Cost ($) Heatmap: Day (rows) vs Hour (columns)");
    ...
    ui.label("Click and drag to select a range to view total Cost");
    ...
    if show_selection_info {
        if let Some(rect) = selection_rect {
            ...
            egui::Area::new("cost_heatmap_selection".into())
                .fixed_pos(pos)
